...
const ListDessert = () => {
  ...
  const onSearchCoffee = async (query, priceMin, priceMax) => {
    try {
      console.log("Searching desserts with:", { query, priceMin, priceMax });
      const response = await axios.get(`${url}/api/dessert/search-dessert`, {
        params: { query, priceMin, priceMax },
      });
      console.log("Search response:", response.data);
      if (response.data && response.data.dessert) {
        setIsSearch(true);
        setData(response.data.dessert);
      } else {
        toast.error("No desserts found");
      }
    } catch (error) {
      console.error("Error searching desserts:", error);
      toast.error("Error occurred while searching for desserts");
    }
  };
  ...
  return (
    <div className="px-6 max-w-7xl mx-auto">
      ...
      {data && data.length > 0 ? (
        ...
        <EmptyCard
          imgSrc={assets.dessert}
          message={
            isSearch
              ? "Oops! No Dessert found matching your search"
              : `Start creating your first Dessert entry! Click the 'Add' button to get started.`
          }
        />
      )}
